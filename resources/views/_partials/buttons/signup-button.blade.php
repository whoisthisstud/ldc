@if( Auth::user() && Auth::user()->cities->first() )
	...
	@if( Auth::user()->cities->count() > 0 && ( Auth::user()->cities->first()->id !== $city->id ) )
		<div class="col-12 text-center">
		    <div class="d-block mb-5">
		        <a href="#" class="btn btn-primary">Add {{ $city->name }}, {{ $city->state->abbreviation }} to your account</a>
		    </div>
		</div>
	@endif
@else
	<div class="col-12 text-center">
	    <div class="d-block mb-5">
	        <a href="{{ route('public.signup', [ 'state' => $city->state->name, 'city' => $city->name ]) }}" class="btn btn-primary">Request Club Card for {{ $city->name }}</a>
	    </div>
	</div>
@endif